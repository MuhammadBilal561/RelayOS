...
import { getBusinessByWidgetKey, getOrCreateConversation, getConversationHistory } from "@/lib/conversations";
...
export async function POST(req: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`widget:${widgetKey}`);
  ...
  try {
    ...
    await supabase.from("messages").insert({
      ...
      role: "assistant",
      content: reply,
      tool_calls: toolCalls.length > 0 ? { calls: toolCalls } : null,
    });
