...
export async function getAnalyticsSummary(businessId: string, days = 30): Promise<AnalyticsSummary> {
  ...
  // Only run batched query if there are conversations to analyze
  if (conversationIds.length > 0) {
    ...
    // Process each conversation's messages to find visitor->assistant pairs
    for (const messages of messagesByConversation.values()) {
      if (messages.length >= 2 && messages[0].role === "visitor" && messages[1].role === "assistant") {
