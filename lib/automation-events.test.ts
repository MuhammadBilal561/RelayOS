...
describe("emitAutomationEvent", () => {
  it("always records the event in automation_events, even with no webhook configured", async () => {
    ...
    chainResults.set("businesses", { data: { n8n_webhook_url: null }, error: null });
  ...
  it("does not attempt delivery when no webhook URL is configured", async () => {
    chainResults.set("automation_events", { data: { id: "evt_1" }, error: null });
    chainResults.set("businesses", { data: { n8n_webhook_url: null }, error: null });
  ...
  it("POSTs the event to the configured webhook and marks it delivered on success", async () => {
    ...
    chainResults.set("businesses", { data: { n8n_webhook_url: "https://n8n.example.com/webhook/relay" }, error: null });
    ...
    expect(fetchSpy).toHaveBeenCalledWith(
      "https://n8n.example.com/webhook/relay",
