...
export async function recalculateLeadScore(leadId: string, conversationId: string): Promise<number> {
  ...
  const [{ data: lead }, { data: visitorMessages }] = await Promise.all([
    supabase.from("leads").select("name, email, phone, service_interest").eq("id", leadId).single(),
    supabase.from("messages").select("content").eq("conversation_id", conversationId).eq("role", "visitor"),
  ]);
  ...
  const score = computeLeadScore({
    ...
    visitorMessageCount: visitorMessages?.length ?? 0,
  });
