  ...
  switch (name) {
    case "capture_lead_info": {
      ...
      if (Object.keys(update).length > 0) {
        const { data: existingLead } = await supabase.from("leads").select("status").eq("id", ctx.leadId).single();
        const wasNew = existingLead?.status === "new";

        update.status = "qualified";
        await supabase.from("leads").update(update).eq("id", ctx.leadId);
    ...
    case "create_booking": {
      ...
      const { data: lead } = await supabase.from("leads").select("email").eq("id", ctx.leadId).single();
    ...
    case "escalate_to_human": {
      ...
      await supabase.from("leads").update({ status: "escalated" }).eq("id", ctx.leadId);
      await supabase
        .from("conversations")
        .update({ status: "escalated", summary_text: summary })
